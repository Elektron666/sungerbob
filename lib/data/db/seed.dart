...
Future<void> seedInitialData(AppDatabase db) async {
  ...
  await db
      .into(db.roles)
      .insert(
        RolesCompanion.insert(id: adminRoleId, code: 'ADMIN', name: 'Patron'),
      );
  await db
      .into(db.roles)
      .insert(
        ...
        .insert(
          ProductsCompanion.insert(
            ...
            defaultWidth: Value(
              p.defaultWidth == null
                  ? null
                  : Dimension.fromStored(p.defaultWidth!),
            ),
            defaultHeight: Value(
              p.defaultHeight == null
                  ? null
                  : Dimension.fromStored(p.defaultHeight!),
            ),
          ),
        );
  }
  ...
  for (final e in defaults.entries) {
    await db
        .into(db.settings)
        .insert(
          SettingsCompanion.insert(key: e.key, value: e.value, updatedAt: now),
        );
  }
  ...
  for (final name in ['Kira', 'Maaş', 'Yakıt', 'Elektrik', 'Diğer']) {
    await db
        ...
        .insert(ExpenseCategoriesCompanion.insert(id: _uuid.v7(), name: name));
  }
}
