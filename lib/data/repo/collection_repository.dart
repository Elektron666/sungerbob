...
final class CollectionRepository {
  ...
  Future<String> _create(CollectionInput input, OperationContext ctx) async {
    ...
    final docNo =
        await db.nextDocumentNumber(DocPrefix.collection, input.docDate.year);
  ...
  Future<void> _allocate(
      String collectionId, CollectionInput input, OperationContext ctx) async {
  ...
  Future<List<({String id, String targetType, Money remaining, int? dueDate})>>
    ...
    final result = <({String id, String targetType, Money remaining, int? dueDate})>[];
    for (final r in rows) {
      final remaining =
          Money.fromStored(r.read<int>('total') - r.read<int>('paid'));
  ...
  Future<String> _createInstrument(
      CollectionInput input, OperationContext ctx) async {
