  ...
  Future<void> changeStatus({
    ...
    String? note,
      ...
      db.runOperation(ctx, () async {
        final inst = await (db.select(db.instruments)
        ...
        if (!allowed.contains(toStatus)) {
          throw InvalidInstrumentTransitionException(
              inst.currentStatus, toStatus);
        }

        await db.into(db.instrumentEvents).insert(InstrumentEventsCompanion.insert(
              id: uuid.v7(),
        ...
        await (db.update(db.instruments)..where((i) => i.id.equals(instrumentId)))
    ...
    switch (toStatus) {
      ...
      case InstrumentStatus.collected:
        if (cashAccountId != null) {
          await db.into(db.accountMovements).insert(AccountMovementsCompanion.insert(
                id: uuid.v7(),
      ...
      case InstrumentStatus.returned:
        if (inst.customerId != null) {
          await db.into(db.customerLedger).insert(CustomerLedgerCompanion.insert(
                ...
                description: Value(toStatus == InstrumentStatus.bounced
      ...
      case InstrumentStatus.paid:
        if (cashAccountId != null) {
          await db.into(db.accountMovements).insert(AccountMovementsCompanion.insert(
                ...
                sourceType: const Value('INSTRUMENT'),
                sourceId: Value(inst.id),
                description: Value('Verilen evrak ödendi: ${inst.serialNo ?? ''}'),
                commandId: Value(ctx.commandId),
                createdAt: ctx.nowMs,
              ));
        }
