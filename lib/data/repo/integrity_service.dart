...
final class IntegrityService {
  ...
  Future<void> _checkBatchRemainders(List<IntegrityFinding> findings) async {
    ...
    for (final r in rows) {
      final expectedPieces =
          r.read<int>('in_pieces') - r.read<int>('consumed_pieces');
      ...
      final expectedVolume = Volume.fromStored(
        r.read<int>('in_volume') - r.read<int>('consumed_volume'),
      );
  ...
  Future<void> _checkVariantStock(List<IntegrityFinding> findings) async {
    final rows = await db
      ...
      ''',
          readsFrom: {db.stockMovements, db.inventoryBatches},
        )
        .get();
  ...
  Future<void> _checkInstrumentStatus(List<IntegrityFinding> findings) async {
    final rows = await db
      ...
      SELECT i.id, i.current_status,
             -- Zincirin sırası KAYIT sırasıdır, iş tarihi değil: vadesi
             -- ileri tarihli bir çek, bugün karşılıksız çıkabilir. UUID v7
             -- sıralanabilir olduğu için id kırıcı olarak yeterli.
      ...
      FROM instruments i
      ''',
          readsFrom: {db.instruments, db.instrumentEvents},
        )
        .get();
