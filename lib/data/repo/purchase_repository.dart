...
final class PurchaseRepository {
  ...
  Future<String> _create(PurchaseInput input, OperationContext ctx) async {
    ...
    final docNo =
        await db.nextDocumentNumber(DocPrefix.purchase, input.docDate.year);
    ...
    for (final line in input.lines) {
      vatLines.add(VatCalculator.forMode(
    ...
    if (input.expenses.isEmpty) {
      ...
      final targets = [
        for (var i = 0; i < input.lines.length; i++)
          ExpenseTarget(volume: input.lines[i].volume, bareCost: vatLines[i].net)
      ];
      final byAmount =
          input.expenses.first.allocationKey == AllocationKey.amount;
      perLineExpense.addAll(CostingEngine.allocatePurchaseExpense(
          expense: expenseTotal, lines: targets, byAmount: byAmount));
    }
    ...
    for (var i = 0; i < input.lines.length; i++) {
      ...
      final realUnitCost = _unitCostOf(vat.net + perLineExpense[i], line.volume);
    ...
    for (final expense in input.expenses) {
      await db.into(db.purchaseExpenses).insert(PurchaseExpensesCompanion.insert(
            id: uuid.v7(),
            purchaseId: purchaseId,
    ...
    await db.into(db.supplierLedger).insert(SupplierLedgerCompanion.insert(
          id: uuid.v7(),
          supplierId: input.supplierId,
          occurredAt: input.docDate.millisecondsSinceEpoch,
    ...
    required OperationContext ctx,
      ...
      db.runOperation(ctx, () async {
        ...
        final batches = await (db.select(db.inventoryBatches)
              ..where((b) => b.sourceId.equals(purchaseId)))
            .get();
        ...
        final shares = CostingEngine.allocatePurchaseExpense(
          ...
          lines: [
            for (final b in batches)
              ExpenseTarget(volume: b.inVolume, bareCost: b.bareUnitCostM3.times(b.inVolume))
          ],
        ...
        for (var i = 0; i < batches.length; i++) {
          ...
          await db.into(db.purchaseExpenseAllocations)
              .insert(PurchaseExpenseAllocationsCompanion.insert(
                id: uuid.v7(),
        ...
        await db.writeAudit(ctx,
            ...
            action: 'LATE_EXPENSE',
            summary:
                'Sonradan masraf eklendi: ${expense.kind} ${expense.amount}');
      });
