  ...
  Future<void> cancelCollection({
    ...
    required OperationContext ctx,
      ...
      db.runOperation(ctx, () async {
        ...
        final original = await (db.select(db.customerLedger)
              ..where((l) =>
        ...
        final movements = await (db.select(db.accountMovements)
              ..where((m) => m.sourceId.equals(collectionId)))
            .get();
        ...
        await (db.update(db.collections)..where((c) => c.id.equals(collectionId)))
