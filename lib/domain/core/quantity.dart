...
final class Dimension {
  ...
  Decimal get cm => (Decimal.fromInt(stored) / Decimal.fromInt(100))
      .toDecimal(scaleOnInfinitePrecision: Scales.dimension);

  @override
  bool operator ==(Object other) => other is Dimension && other.stored == stored;
  @override
...
final class Volume implements Comparable<Volume> {
  ...
  factory Volume.fromDecimal(Decimal m3) => Volume(
      (roundHalfUp(m3, Scales.volume) * _factor).toBigInt().toInt());
  ...
  factory Volume.fromDimensions({
    ...
    final product = Decimal.fromBigInt(BigInt.from(width.stored) *
        BigInt.from(height.stored) *
        BigInt.from(thickness.stored) *
        BigInt.from(pieces));
    final scaled = (product / Decimal.fromInt(1000000))
  ...
  Decimal get m3 => (Decimal.fromInt(stored) / _factor)
      .toDecimal(scaleOnInfinitePrecision: Scales.volume);
...
final class UnitPrice implements Comparable<UnitPrice> {
  ...
  Decimal get perM3 => (Decimal.fromInt(stored) / _factor)
      .toDecimal(scaleOnInfinitePrecision: Scales.unitPrice);
  ...
  @override
  bool operator ==(Object other) => other is UnitPrice && other.stored == stored;
  @override
  int get hashCode => stored.hashCode;
  @override
...
final class Rate {
  ...
  factory Rate.percent(String percent) => Rate(
      (Decimal.parse(percent) * Decimal.fromInt(100)).toBigInt().toInt());
