...
void main() {
  ...
  test('domain ve data katmanında double/num kullanılmıyor', () {
    ...
    expect(
      offenders,
      isEmpty,
      reason: 'Para ve m³ hesabında double YASAK (BRIEF §2). Bulunanlar:\n'
          '${offenders.join('\n')}',
    );
  });
  ...
  test('domain katmanı Flutter\'a bağımlı değil', () {
    ...
    expect(offenders, isEmpty,
        reason: 'domain saf Dart kalmalı (BRIEF §2). Bulunanlar:\n'
            '${offenders.join('\n')}');
  });
  ...
  test('yuvarlama yalnızca rounding.dart içinde tanımlı', () {
    ...
    expect(offenders, isEmpty,
        reason: 'Tek yuvarlama kaynağı roundHalfUp (BRIEF §3.3). Bulunanlar:\n'
            '${offenders.join('\n')}');
  });
}
